...
def main(config_name):
    ...
    if analysis_flags[0]:  # Convert new MEDPC files to neo
        ...
        in_files = bvmpc.bv_utils.get_all_files_in_dir(
            in_dir, return_absolute=True)
        ...
        if not using_axona:
            for filename in in_files:
                try:
                    ...
                    bvmpc.bv_utils.log_exception(
                        e, "Error during coversion to neo")
        else:
            for filename in in_files:
                try:
                    if os.path.splitext(filename)[-1] == ".inp":
