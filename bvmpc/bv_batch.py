    ...
    for sub in sub_list:
        ...
        for i, s in enumerate(s_grp):
            ...
            if "B" in s_name:
                ...
                norm_r_ts, _, norm_err_ts, norm_dr_ts, _ = s.split_sess(all_levers=True)
...
def plot_batch_sessions(
    ...
    if plt_flags["raster"] == 1 or plt_flags["hist"] == 1:
        ...
        for j, plot_grp in enumerate(plot_set):
            ...
            df_sub, df_date, df_stage = (
                [],
                [],
                [],
            )  # Initialize plot naming parameters
            ...
            for i, s in enumerate(plot_grp):  # Iterate through groups to plot rasters
                ...
                if plt_flags["raster"] == 1:
                    ...
                    bv_an.plot_raster_trials(s, ax, sub_colors_dict, align=[0, 0, 1, 0])
        ...
        if not single:
            ...
            for l in sub_list:
                timeline_plot(
                    ...
                    show_date=show_date,
                    details=details,
                    sub_colors_dict=sub_colors_dict,
                )
        else:
    ...
    if single and summary:
        ...
        for sub in sub_list:
            ...
            if "7" in s_passed:
                ...
                fig.suptitle(
                    ("Subject " + subject + " Performance"), color=color, fontsize=30,
                )
...
def sum_plot(
    s_grp, idx, out_dir, zoom=True, single=False, int_only=False, corr_only=False,
):
    """
    ...
    for _, s in enumerate(s_grp):
        ...
        if corr_only and stage == "7":
            bv_an.cumplot(
                s, out_dir, ax1, int_only, zoom=False, zoom_sch=False, plot_all=False,
            )
        else:
            bv_an.cumplot(s, out_dir, ax1, int_only, zoom=False, zoom_sch=False)
    ...
    if single:
        ...
        if idx == 0:
            fig.suptitle(
                ("Summary across animals " + d_title + "_Correct Only"), fontsize=30,
            )
        ...
        if idx == 0:
            ...
            fig.suptitle(
                ("Summary across animals " + d_title + " p" + str(idx)), fontsize=30,
            )
...
def timeline_plot(
    ...
    size_multiplier = 5
    fig = plt.figure(
        figsize=(cols * size_multiplier, rows * size_multiplier), tight_layout=False,
    )
    ...
    for c, sub in enumerate(sub_list):
        ...
        if (
            details
        ):  # Plots average sessions variables i.e. FR_corr, FI_corr, FR_err, FI_err, double_rw
            ...
            ax.set_title(
                "\nSubject {} Timeline_Details".format(subject),
                y=1.05,
                fontsize=25,
            ...
            ax.set_title(
                "\nSubject {} Timeline".format(subject),
                y=1.05,
                fontsize=25,
                color=bvmpc.bv_utils.mycolors(subject, sub_colors_dict),
            )
        ...
        if details:
            if "S-6" in type_list:
                ax.axhline(
                    45,
                    xmax=(type_list.index("S-6") / len(type_list)),
                    color=interval_c(2 * 45),
                    linestyle="-.",
                    linewidth=".5",
                )
                ax.axhline(
                    30,
                    xmin=(type_list.index("S-6") / len(type_list)),
                    color=interval_c(2 * 45),
                    linestyle="-.",
                    linewidth=".5",
                )
                ...
                ax.axhline(30, color=interval_c(2 * 45), linestyle="-.", linewidth=".5")
                ax.text(
                    s_idx[0],
                    31,
                    " Max FI",
            ...
            plt.axhline(
                60, color="r", linestyle="-.", linewidth=".5"
            )  # Marks max reward
