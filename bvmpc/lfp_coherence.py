...
def mean_vector_length(low_freq_lfp, high_freq_lfp, amp_norm=True, return_all=False):
    """
...
def mvl_shuffle(low_freq_lfp, high_freq_lfp, amp_norm=True, nshuffles=200):
    ...
    observed_mvl = mean_vector_length(low_freq_lfp, high_freq_lfp, amp_norm=amp_norm)
    shuffled_mvl = np.zeros(shape=(nshuffles))
    for i in range(len(shuffled_mvl)):
        sample_idx = int(
            np.floor((low_freq_lfp.get_total_samples() + 1) * np.random.random_sample())
        )
        reversed_arr1 = samples[0:sample_idx][::-1]
        reversed_arr2 = samples[sample_idx : samples.size][::-1]
        permuted_amp_time = np.concatenate([reversed_arr1, reversed_arr2], axis=None)
...
def plot_wave_coherence(
    ...
    if np.max(WCT) > 1 or np.min(WCT) < 0:
        print("WCT was out of range: min {},max {}".format(np.min(WCT), np.max(WCT)))
        WCT = np.clip(WCT, 0, 1)
    ...
    if sig:
        ax.contour(
            t, y_vals, sig, [-99, 1], colors="k", linewidths=2, extent=extent_corr,
        )
    ...
    if plot_arrows:
        ...
        if all_arrows:
            ...
            sub_u = u[::y_res, ::x_res][
                np.array(high_points[0]), np.array(high_points[1])
            ]
    ...
    if plot_period:
        ...
        # TODO improve ticks
        y_ticks = [
            np.log2(x) for x in [0.004, 0.008, 0.016, 0.032, 0.064, 0.125, 0.25, 0.5, 1]
        ]
...
def calc_wave_coherence(
    wave1, wave2, sample_times, min_freq=1, max_freq=128, sig=False, resolution=12,
):
    """
    Calculate wavelet coherence between wave1 and wave2 using pycwt.
...
def plot_arrows(
    ...
    if all_arrows:
        ...
        sub_t = t[::x_res][high_points[1]]
        sub_y = y_vals[::y_res][high_points[0]]
        sub_u = u[::y_res, ::x_res][np.array(high_points[0]), np.array(high_points[1])]
        sub_v = v[::y_res, ::x_res][high_points[0], high_points[1]]
        res = 1
        ax.quiver(
            sub_t[::res],
...
def wcohere_mean(WCT, aWCT, t_blocks=None):
    ...
    WCT_trial = np.empty((len(t_blocks), WCT.shape[0], t_win), dtype=np.float32)
    all_u = np.empty((len(t_blocks), u.shape[0], t_win), dtype=np.float32)
    all_v = np.empty((len(t_blocks), v.shape[0], t_win), dtype=np.float32)
    ...
    for i, (a, b) in enumerate(t_blocks):
        ...
        WCT_this_trial = np.empty((WCT.shape[0], t_win), dtype=np.float32)
        u_this_trial = np.empty((u.shape[0], t_win), dtype=np.float32)
        v_this_trial = np.empty((v.shape[0], t_win), dtype=np.float32)
...
def plot_single_freq_wcohere(
    ...
    for i, (a, b) in enumerate(trials):
        ...
        t_freq_WCT[i, start:] = WCT[freq_idx, a_idx:b_idx]
        t_freq_aWCT[i, start:] = aWCT[freq_idx, a_idx:b_idx]

    # Generate pd.df for WCT & aWCT
    t_WCT_df = pd.DataFrame(
        t_freq_WCT,
        columns=np.round((np.arange(t_win[0], t_win[1], 0.004)), decimals=3),
    ...
    for g in grp_dict.keys():
        grp_dict[g]["WCT"] = t_WCT_df.loc[grp_ref == g, t_win[0] : t_win[1]]
        grp_dict[g]["aWCT"] = t_aWCT_df.loc[grp_ref == g, t_win[0] : t_win[1]]
    ...
    if dist:
        ...
        fig2, ax = plt.subplots(
            1, len(dist_range), figsize=(len(dist_range) * 5, 2), sharey=True
        )
        ...
        for i, a in enumerate(dist_range):
            for g, val in grp_dict.items():
                ...
                sns.distplot(val["aWCT"].loc[:, (start_m & end_m)], ax=ax[i], label=g)
            ax[i].text(
                0.5,
                1.05,
                "{} to {} s from {}".format(a, a + step, align_txt),
                fontsize=8,
                va="center",
                ha="center",
        ...
        fig2.suptitle(
            "{} vs {} wCohere Phase Distribution ({}Hz) - FR vs FI".format(
                reg_sel[0], reg_sel[1], target_freq
            ),
            y=1.08,
            ha="center",
            va="center",
    ...
    if plot:
        ...
        fig.suptitle(
            "{} vs {} Trial-based wCohere ({}Hz) - FR vs FI".format(
                reg_sel[0], reg_sel[1], target_freq
            ),
            y=0.92,
            ha="center",
        ...
        for i, (g_name, g) in enumerate(grp_dict.items()):
            if i == 0:
                sns.heatmap(g["WCT"], ax=ax[i], xticklabels=int(250), cbar_ax=axlab)
                ax[i].set_title(s.get_title(), fontsize=10)
            else:
                sns.heatmap(g["WCT"], ax=ax[i], xticklabels=int(250), cbar=False)
            ...
            ax[i].quiver(
                X,
                Y + 0.5,
                g["u"].mask(res_mask, np.nan),
                g["v"].mask(res_mask, np.nan),
                units="height",
                angles="uv",
                pivot="mid",
                edgecolor="k",
        ...
        ax[-1].axvline(0, linestyle="-.", color="k", linewidth=1, label=align_txt)
        ...
        if grp_ref.name in ["Schedule", "Sch_block"]:
            ...
            gm = bv_plot.GroupManager(
                s.get_arrays("Trial Type").tolist(), color_list=color_list
            )
            colors = []
        ...
        plot_data = pd.melt(
            t_WCT_df, id_vars=["Groups"], var_name="Time (s)", value_name="Coherence",
        )
        sns.lineplot(
            x="Time (s)",
            y="Coherence",
...
def plot_cross_wavelet(
    ...
    if np.max(W12) > 6 ** 2 or np.min(W12) < 0:
        print("W12 was out of range: min{},max{}".format(np.min(W12), np.max(W12)))
...
def test_mvl():
    ...
    plt.show()
    plt.close("all")
    all1, res1 = mean_vector_length(sig1, coupled_sig, amp_norm=False, return_all=True)
...
if __name__ == "__main__":
    ...
    if test_record:
        ...
        f, Cxy = calc_coherence(lfp_odict.get_signal(0), lfp_odict.get_signal(1))
        from lfp_plot import plot_coherence
