...
def cumplot_axona(s, ax=None, p_errors=False, p_all=False):
    ...
    norm_r_ts, norm_l_ts, norm_err_ts, norm_dr_ts, incl = s.split_sess(
        blocks=blocks, error_only=p_errors, all_levers=p_all
    )
    ...
    ax[0].step(
        lever_times, np.arange(lever_times.size), where="post", label=sub, zorder=1,
    )
    if stage == "7":  # plots error press in red
        ax[0].scatter(
            err_lever_ts,
            np.isin(lever_times, err_lever_ts).nonzero()[0],
            c="r",
            label="Errors",
            s=1,
            zorder=2,
    ...
    ax[0].scatter(
        reward_times, reward_y, marker="x", c="grey", label="Reward Collected", s=25,
    )
    ...
    norm_r_ts, _, norm_err_ts, _, _ = s.split_sess(blocks=blocks, all_levers=True)
    sch_type = s.get_arrays("Trial Type")
    ...
    rw_print = "\nCorrect FR \\ FI: " + str(rw_FR) + r" \ " + str(rw_FI)
    ...
    text = "Total # of Lever Press: " + "{}\nTotal # of Rewards: {}{}{}{}".format(
        len(lever_ts),
        len(reward_times) + len(reward_double),
        dr_print,
        rw_print,
        err_print,
    )
    ax[0].text(0.03, 0.97, text, ha="left", va="top", transform=ax[0].transAxes)
    ...
    for i, l in enumerate(norm_l_ts):
        ...
        if stage == "7" and p_all:  # plots all responses incl. errors
            ax[1].scatter(
                norm_err_ts[i],
                np.isin(l, norm_err_ts[i]).nonzero()[0],
                c="r",
                s=1,
                zorder=2,
            )
        ...
        ax[1].scatter(norm_r_ts[i], reward_y, marker="x", c="grey", s=25)
        ax[1].scatter(norm_dr_ts[i], double_y, marker="x", c="magenta", s=25)
    ax[1].set_title("{}, Block-Split {}".format(sub, incl), fontsize=10)
    ax[1].legend(loc="upper left")
    fig.suptitle("{} {} S{}".format(sub, date, stage), y=0.99, fontsize=20)
    return fig
...
def cumplot(
    ...
    if ax is None:
        ...
        if session_type == "5a_FixedRatio_p":
            plt.suptitle(
                "\n{}, {} {}, {}".format(subject, session_type[:-2], ratio, date),
            ...
            plt.suptitle(
                "\n{}, {} {}s, {}".format(subject, session_type[:-2], interval, date),
                color=mycolors(subject),
                fontsize=10,
                y=0.98,
                x=0.51,
        ...
        if session_type == "5a_FixedRatio_p":
            ax.set_title(
                "\n{}, S{}, FR{}, {}".format(subject, stage, ratio, date),
                color=mycolors(subject),
                fontsize=10,
            )
        elif session_type == "5b_FixedInterval_p":
            ax.set_title(
                "\n{}, S{}, FI{}s, {}".format(subject, stage, interval, date),
                color=mycolors(subject),
                fontsize=10,
            )
    ...
    if zoom:
        ...
        ax.set_title(
            "\n{}, Trial-Based".format(subject), color=mycolors(subject), fontsize=10,
        )
        ...
        for i, l in enumerate(norm_l_ts):
            ...
            if int_only:
                if sch_type[i] == 0:
                    plt.scatter(norm_r_ts[i], reward_y, marker="x", c="grey", s=25)
                    plt.scatter(norm_dr_ts[i], double_y, marker="x", c="magenta", s=25)
            else:
                plt.scatter(norm_r_ts[i], reward_y, marker="x", c="grey", s=25)
                plt.scatter(norm_dr_ts[i], double_y, marker="x", c="magenta", s=25)
        ax.set_title(
            "\n{}, Block-Split {}".format(subject, incl),
            color=mycolors(subject),
            fontsize=10,
        )
        ...
        if session_type == "5a_FixedRatio_p":
            ...
            sch_type = "FI"
            ax.set_title(
                "\n{}, FI{}s Split".format(subject, interval),
                color=mycolors(subject),
                fontsize=10,
            )
        ...
        norm_r_ts, norm_l_ts, norm_err_ts, norm_dr_ts, _ = session.split_sess(blocks)
        ax.set_xlim(0, 305)
        for i, l in enumerate(norm_l_ts):
            ax.step(
                l,
        ...
        double_y = np.digitize(reward_double, bins) - 1
        # for printing of error rates on graph
        norm_r_ts, _, norm_err_ts, _, _ = session.split_sess(all_levers=True)
        sch_type = session.get_arrays("Trial Type")
        for i, l in enumerate(norm_err_ts):
            if sch_type[i] == 1:
                err_FR = err_FR + len(norm_err_ts[i])
...
def IRT(session, out_dir, ax=None, showIRT=False):
    ...
    if ax is None:
        ...
        if session_type == "5a_FixedRatio_p":
            ...
            plt.suptitle(
                "\n({}, {}, {})".format(subject, session_type[:-2], date),
                fontsize=10,
                y=0.98,
                x=0.51,
            )
    ...
    if single_plot:
        ...
        out_name = (
            subject.zfill(3) + "_IRT_Hist_" + session_type[:-2] + "_" + date + ".png"
        )
...
def lever_hist(
    s, ax=None, valid=True, excl_dr=False, split_t=False, sub_colors_dict=None
):
    """
    ...
    if split_t:
        ...
        for idx, row in t_df.iterrows():
            ...
            if row["Schedule"] == "FR":
                sns.distplot(
                    x, ax=ax, label="FR-t{}".format(idx + 1), color=color, hist=True,
                )
            elif row["Schedule"] == "FI":
                sns.distplot(
                    x, ax=ax, label="FI-t{}".format(idx + 1), color=color, hist=True,
                )
        legend_size = 6
    else:
    ...
    ax.set_title(
        "  {}".format(plot_name), y=1.04, ha="center", fontsize=25, color=color
    )
    ax.text(
        0.5,
        1.015,
        "{} {} S{}".format(sub, date, stage),
        ha="center",
        transform=ax.transAxes,
        fontsize=12,
        color=color,
...
def plot_raster_trials(
    ...
    ax.axvline(0, linestyle="-.", color="g", linewidth=1)
    ax.text(
        0.1, -1, plot_type.split("-")[0], fontsize=12, color="g", ha="left", va="top",
    )
    ...
    ax.set_title("  {}".format(plot_name), y=1.04, ha="center", fontsize=20)
    ax.text(
        0.5,
        1.015,
        "{} {} S{}".format(sub, date, stage),
    ...
    if opt_plot[2]:
        for i, (x, sch) in enumerate(zip(norm_start, schedule_type)):  # Plot DR window
            if sch == "FI":
                plt.broken_barh(
                    [(x + 20, 20)], (i - 0.4, 0.8), color="magenta", alpha=0.05
                )
        drwin_label = mpatches.Patch(color="magenta", alpha=0.05, label="dr_win")
...
def trial_clustering(s, ax=None, should_pca=False, num_clusts=2, p_2D=False):
    ...
    if zs is None:
        sns.scatterplot(
            xs, ys, ax=ax, style=markers, hue=cluster.labels_, palette="bright"
        )
        ...
        for i in np.arange(len(xs)):
            ...
            ax.scatter(xs[i], ys[i], zs[i], marker=mask[markers[i]], c=[ic], s=40)
            PCA_colors.append(ic)
        ax.set_zlabel("PC3")
        ax.dist = 10
        legend_c = [
            lines.Line2D(
                [], [], color=cmap[c], ls="", marker="s", label="c{}".format(c)
            )
            for c in np.unique(cluster.labels_)
        ]
        legend_c.append(lines.Line2D([], [], color="k", ls="", marker="x", label="FR"))
        legend_c.append(lines.Line2D([], [], color="k", ls="", marker="^", label="FI"))
    ...
    if zs is None:
        ax.text(
            0.5, 1.015, s.get_title(), ha="center", transform=ax.transAxes, fontsize=12,
        )
...
def plot_feats(feat_df, ax=None):
    ...
    ax.set_title("Cluster Features Boxplot", y=1.04, ha="center", fontsize=25)
...
def trial_clust_hier(s, ax=None, should_pca=True, cutoff=None):
    ...
    ax[0].set_title("  {}".format(plot_name), y=1.04, ha="center", fontsize=20)
    ax[0].text(
        0.5,
        1.015,
        "{} {} S{}".format(sub, date, stage),
...
def test_matlab_wcoherence(lfp1, lfp2, rw_ts, sch_n, reg_sel=None, name="default.png"):
    ...
    title = "{} vs {} Wavelet Coherence {}".format(reg_sel[0], reg_sel[1], sch_n)
    eng.title(title)
...
def test_wct(lfp1, lfp2, sig=True):  # python CWT
    ...
    sig95 = WCT / sig95
    ax.contour(t, freq, sig95, [-99, 1], colors="k", linewidths=2, extent=extent)
    ax.fill(
        np.concatenate([t, t[-1:] + dt, t[-1:] + dt, t[:1] - dt, t[:1] - dt]),
        np.concatenate([coi, [1e-9], freq[-1:], freq[-1:], [1e-9]]),
        "k",
        alpha=0.3,
        hatch="x",
    )
