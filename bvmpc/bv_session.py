...
class Session:
    ...
    def split_sess(self, norm=True, blocks=None, error_only=False, all_levers=False):
        """
        ...
        reward_double = reward_times[
            np.searchsorted(reward_times, pell_ts[pell_double], side="right")
        ]
        err_lever_ts = []
        ...
        if stage == "7" and error_only:  # plots errors only
            ...
            lever_ts = np.sort(np.concatenate((lever_ts, err_lever_ts), axis=None))
        ...
        split_double_r_ts = np.split(
            reward_double, np.searchsorted(reward_double, blocks)
        )
        split_err_ts = np.split(err_lever_ts, np.searchsorted(err_lever_ts, blocks))
        norm_reward_ts = []
        norm_lever_ts = []
        norm_err_ts = []
        norm_double_r_ts = []
    ...
    def old_extract_features(self, should_scale=True):
        ...
        features = np.zeros(shape=(len(df.index), len(feat_names)))
        ...
        for row in df.itertuples():
            ...
            features[index, 5] = np.count_nonzero(~np.isnan(row.Levers_ts))
            ...
            lever_hist = np.histogram(x[~np.isnan(x)], bins=h_bin_final, density=True)[
                0
            ]
    ...
    def extract_features(self, should_scale=True):
        ...
        for row in df.itertuples():
            ...
            # Number of lever responses
            features[index, 1] = np.count_nonzero(~np.isnan(row.Levers_ts))
            ...
            for value in press_times:
                in_range = np.logical_and(
                    press_times >= value - dt, press_times <= value + dt
                )
    ...
    def perform_pca(self, n_components=5, should_scale=True):
        ...
        after_pca = pd.DataFrame(
            after_pca,
            columns=["PC{}".format(x) for x in np.arange(after_pca.shape[1]) + 1],
        )
    ...
    def perform_HDBSCAN(self, should_scale=True):
        ...
        _, data, _ = self.perform_pca(n_components=None, should_scale=should_scale)
        ...
        clusterer.condensed_tree_.plot(
            ...
            selection_palette=sns.color_palette("deep")[1:],
            axis=ax[1],
        )  # Plots condensed dendogram
        plt.suptitle("HDBSCAN", fontsize=24)
        fig.text(0.5, 0.91, self.get_title(), transform=fig.transFigure, ha="center")
    ...
    def perform_UMAP(self, should_scale=True):
        ...
        for n in (2, 5, 10, 20, 50, 100, 200):
            self.draw_umap(
                data, color, n_neighbors=n, title="n_neighbors = {}".format(n)
            )
        plt.show()
        exit(-1)
    ...
    def hier_cluster(
        self, should_scale=True, should_pca=True, cutoff=None, test_methods=False,
    ):
        """
        ...
        """
        if should_pca:
            _, data, _ = self.perform_pca(n_components=5)  # Use PCA as features
        else:
            data = self.extract_features(should_scale=should_scale)  # Use raw features
    ...
    def save_to_neo(self, out_dir, name=None, neo_backend="nix", remove_existing=False):
    ...
    def get_lever_ts(self, include_un=True):
        ...
        """
        levers = [self.get_arrays("R"), self.get_arrays("L")]
        if include_un:
    ...
    def get_one_lever_ts(self, lever_type, include_un=True):
        ...
        if (lever_type != "L") and (lever_type != "R"):
            raise ValueError(
                "lever_type is expected to be L or R, entered {}".format(lever_type)
            )
    ...
    def get_err_lever_ts(self, include_un=True):
        ...
        """
        levers = [self.get_arrays("FR_Err"), self.get_arrays("FI_Err")]
        if include_un:
    ...
    def get_rw_ts(self):
        ...
        if stage == "7" or stage == "6":
            ...
            split_pell_ts = split_into_blocks(pell_ts_exdouble, blocks=block_ends)
            split_reward_ts = split_into_blocks(reward_times, blocks=block_ends)

            for i, (pell, reward) in enumerate(zip(split_pell_ts, split_reward_ts)):
                if len(pell) > len(reward):
                    block_len = block_ends[i]
                    reward_times = np.insert(
                        reward_times,
                        np.searchsorted(reward_times, block_len),
                        block_len,
                    )
    ...
    def _save_neo_info(self, remove_existing):
        ...
        if os.path.isfile(self.neo_file):
            if remove_existing:
                ...
                print(
                    "Skipping {} as it already exists".format(self.neo_file)
        ...
        for key, val in self.get_arrays().items():
            e = Event(times=val * s, labels=None, name=key, nix_name="Event_" + key)
            seg.events.append(e)
    ...
    def _extract_neo_info(self):
        ...
        for event in block.segments[0].events:
            key = event.name
            self.info_arrays[key] = event.times.rescale("s").magnitude
    ...
    def _get_neo_io(self, get_ext=False):
        ...
        if backend == "nix":
            ...
            print("Backend {} not recognised, defaulting to nix".format(backend))
            from neo.io import NixIO
    ...
    def _extract_axona_info(self):
        ...
        self.metadata["subject"] = os.path.basename(self.axona_file).split("_")[0]
        ...
        for k, v in self.session_info.get_input_channel(self.s_type).items():
            self.info_arrays[k] = self.axona_info.get_times("I", v[0], v[1])
        for k, v in self.session_info.get_output_channel(self.s_type).items():
            self.info_arrays[k] = self.axona_info.get_times("O", v[0], v[1])
    ...
    def _convert_axona_info(self):
        ...
        left_presses = check_error_during_tone(left_presses, block_s, l_type=" left ")
        ...
        split_nosepokes = split_into_blocks(good_nosepokes, blocks=block_ends)
        split_pellets = split_into_blocks(pell_ts_exdouble, blocks=block_ends)
        split_all_nosepokes = split_into_blocks(nosepokes, blocks=block_ends)
        for i in range(6):
            ...
            if len(b2) > len(b1):
                ...
                nosepokes = np.insert(nosepokes, last_nosepoke_idx + 1, to_insert)
                good_nosepokes, un_nosepokes = split_array_with_another(
                    nosepokes, pell_ts_exdouble
                )
                # if i < 5:
        ...
        for i, (b1, b2) in enumerate(zip(split_nosepokes, split_pellets)):
            if b2.shape != b1.shape:
                ...
                print(
                    "{} nosepokes, {} pellets, in block {}".format(
                        len(b1), len(b2), i + 1
                    )
                )
                exit(-1)
        ...
        split_left_presses = split_into_blocks(left_presses, blocks=block_ends)
        ...
        split_right_presses = split_into_blocks(right_presses, blocks=block_ends)
    ...
    def _extract_session_arrays(self):
        ...
        data_info = self.session_info.get_session_type_info(self.get_metadata("name"))
    ...
    def _init_trial_df(self):
        ...
        trial_err_ts = np.split(err_ts, (np.searchsorted(err_ts, trial_starts)[1:]))
        trial_dr_ts = np.split(dpell, (np.searchsorted(dpell, trial_starts)[1:]))
        # Tone end ts
        trial_tone_end = np.split(block_s, (np.searchsorted(block_s, trial_starts)[1:]))
    ...
    def _extract_array(lines, start_char, end_char):
        ...
        arr = np.empty(5 * (len(data_lines) - 1) + len(last_line), dtype=np.float32)
    ...
    def __repr__(self):
        ...
        """
        return self.get_metadata("start_date") + " " + self.get_subject_type()
